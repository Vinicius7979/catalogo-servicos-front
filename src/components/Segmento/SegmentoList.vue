...
defineProps<{
  segmentos: { uuid: string; descricao: string; ativo: boolean }[]
}>()
</script>
...
<template>
    <table class="min-w-full divide-y divide-neutral-200">
        ...
        <tbody class="bg-white divide-y divide-neutral-100">
            <tr v-for="(seg, index) in segmentos" :key="seg.uuid ?? (seg.descricao + '-' + index)" class="hover:bg-neutral-50 transition">
                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-neutral-800">{{ seg.descricao }}</td>
                <td class="px-6 py-4 whitespace-nowrap text-sm">
                    <button 
                        @click="$emit('editar', seg)"
                ...
                <td class="px-6 py-4 whitespace-nowrap text-sm">
                    <button 
                        @click="$emit('remover', seg.uuid ?? seg.descricao)" 
