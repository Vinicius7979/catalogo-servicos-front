...
defineProps<{
    setores: { uuid: string; descricao: string; ativo: boolean} []
}>();
...
<template>
    <table class="min-w-full divide-y divide-neutral-200">
      ...
      <tbody class="bg-white divide-y divide-neutral-100">
        <tr v-for="(set, index) in setores" :key="set.uuid ?? (set.descricao + '-' + index)" class="hover:bg-neutral-50 transition">
          <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-neutral-800">{{ set.descricao }}</td>
          <td class="px-6 py-4 whitespace-nowrap text-sm">
            <button 
              @click="$emit('editar', set)"
          ...
          <td class="px-6 py-4 whitespace-nowrap text-sm">
            <button 
              @click="$emit('remover', set.uuid ?? set.descricao)" 
