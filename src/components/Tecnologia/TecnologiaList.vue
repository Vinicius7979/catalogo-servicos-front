...
defineProps<{
  tecnologias: { uuid: string; descricao: string; versao: string; ativo: boolean }[]
}>()
</script>
...
<template>
    ...
    <tbody class="bg-white divide-y divide-neutral-100">
      <tr v-for="(tec, index) in tecnologias" :key="tec.uuid ?? (tec.descricao + '-' + index)" class="hover:bg-neutral-50 transition">
        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-neutral-800">{{ tec.descricao }}</td>
        <td class="px-6 py-4 whitespace-nowrap text-sm text-neutral-700">{{ tec.versao }}</td>
        <td class="px-6 py-4 whitespace-nowrap text-sm">
          <button 
            @click="$emit('editar', tec)"
        ...
        <td class="px-6 py-4 whitespace-nowrap text-sm">
          <button 
            @click="$emit('remover', tec.uuid ?? tec.descricao)" 
